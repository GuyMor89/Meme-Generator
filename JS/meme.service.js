...
function getImgArray() {
    ...
    const pageObject = pageBy.find(obj => obj.page === 'gallery')

    pageObject.totalAmount = imageArray.length

    const { pageStart, pageEnd } = setPage(pageObject)
...
function getMemeArray() {
    ...
    const pageObject = pageBy.find(obj => obj.page === 'meme')

    pageObject.totalAmount = memeArray.length
...
function setPage(pageObject) {
    const pageStart = (pageObject.currentPage * pageObject.amountPerPage)
    const pageEnd = ((pageObject.currentPage * pageObject.amountPerPage) + pageObject.amountPerPage - 1)
